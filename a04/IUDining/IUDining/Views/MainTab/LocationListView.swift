...
/// Main dining locations list screen (first tab).
///
/// Displays a searchable, pull-to-refresh list of all dining locations.
/// Each row shows the location's name, open/closed status, next transition
/// time, and distance from the user. Tapping a row navigates to the
/// location's detail view.
struct LocationListView: View {
    @State private var viewModel = LocationListViewModel()
    @State private var searchText = ""

    /// Locations filtered by the current search text (case-insensitive).
